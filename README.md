...
인덱스·HikariCP 튜닝은 DB 응답 시간과 커넥션 대기를 줄여 **평균 레이턴시와 처리량**을 개선한다.
그러나 고부하 구간에서 락 경합이 증가하면 요청들이 Redisson `waitTime` 상한(3 s)에 가까운 시간을 대기하게 되고,
이 대기 시간이 p95 구간을 채우기 때문에 **DB 계층 최적화의 효과는 tail latency에 반영되지 않는다.**
